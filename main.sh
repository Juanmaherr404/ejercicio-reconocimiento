...
ctfr -d "$dominio" > "$ruta_resultados/raw/resultado_ctfr.txt" 2>&1
wait  # Esperar a que CTFR termine

# Ejecutar Katana para encontrar subdominios y guardar los resultados en raw/subdominios.txt
echo "Ejecutando Katana para encontrar subdominios..."
katana -u "$dominio" > "$ruta_resultados/raw/resultado_katana.txt" 2>&1
wait  # Esperar a que Katana termine

cat "$ruta_resultados/raw/resultado_katana.txt" "$ruta_resultados/raw/resultado_ctfr.txt" | sort | uniq | httpx -silent > "$ruta_resultados/clean/resultado_subdominios.txt" 2>&1
...
# Ejecutar NMAP para encontrar analizar Ips y guarda informacion en /raw/nmap.
echo "Ejecutando NMAP para encontrar informacion de IPs y puerto abiertos..."
nmap -sT -iL "$ruta_resultados/clean/IP" > "$ruta_resultados/raw/resultados_nmap.txt" 2>&1
wait  # Esperar a que NMAP termine
